...
import {
  ...
  FiTrash2, FiTag, FiX, FiSave, FiEdit3
} from "react-icons/fi";
...
export default function Notes() {
  ...
  const autoSave = (field, value) => {
    ...
    saveTimeout.current = setTimeout(() => {
      if (!isNew && active) {
        api.put(`/notes/${active._id}`, {
          title: field === "title" ? value : form.title,
          content: field === "content" ? value : form.content,
          tags: (field === "tags" ? value : form.tags).split(",").map(t => t.trim()).filter(Boolean),
        }).catch(() => {});
      }
    }, 1200);
  };
          ...
          <div className="notes-list">
            {loading ? (
              <div className="notes-skeleton">
                {[1, 2, 3].map(i => <div key={i} className="note-skeleton-item" />)}
              </div>
            ) : notes.length === 0 ? (
        ...
        <main className="notes-editor">
          {active || isNew ? (
            <>
              <div className="editor-toolbar">
                ...
                <div className="editor-toolbar-actions">
                  ...
                  data-testid="save-note-button" onClick={save}>
                    <FiSave /> Save
                  </button>
                  <button className="btn btn-light btn-sm" 
                   data-testid="close-note-button"onClick={close}>
                    <FiX />
                  </button>
                </div>
              </div>
            ...
            <div className="editor-empty">
              ...
              <p>Pick a note from the list or hit <strong>New</strong> to start writing.</p>
